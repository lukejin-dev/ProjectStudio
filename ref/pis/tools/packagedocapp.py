...
class PackageDocMainFrame(wx.Frame):
    def __init__(self, parent, title):
        wx.Frame.__init__(self, parent, -1, title, size=(490, 400), style=wx.MINIMIZE_BOX|wx.SYSTEM_MENU|wx.CAPTION|wx.CLOSE_BOX )
        ...
        self._archCtrl = wx.ComboBox(panel, -1, value='ALL', choices=['ALL', 'IA32/Microsoft', 'IA32/GCC', 'X64/Microsoft', 'X64/GCC', 'IPF/Microsoft', 'IPF/GCC', 'EBC/Intel'], 
        ...
        self._allMacroCtrl = wx.TextCtrl(panel, -1, 'MDE_CPU_IA32 MDE_CPU_X64 MDE_CPU_EBC MDE_CPU_IPF _MSC_EXTENSIONS __GNUC__ __INTEL_COMPILER ASM_PFX= OPTIONAL= ', 
        ...
        self._generateBt = GB.GradientButton(panel, -1, None, "Generate Package Document!")
    ...
    def CheckInput(self):
        ...
        if len(dPath) == 0 or not os.path.exists(dPath):
            self._Error("Can not find doxygen tool! Please download it from www.stack.nl/~dimitri/doxygen/download.html" % dPath)
            return False
        else:
...
class ProgressDialog(wx.Dialog):
    def __init__(self, parent, id=wx.ID_ANY):
        ...
        self._closeBt    = GB.GradientButton(self, -1, None, "Close")
        self._gotoOuputBt = GB.GradientButton(self, -1, None, "Goto Output")
    ...
    def GenerateAction(self):
        ...
        pObj.Load(pkPath)
        #except:
        #    self._ErrorDialog("Fail to create package object model!")
        #    self._isBusy = False
        #    return
        self._pObj = pObj
        ...
        try:
            ...
            self._ErrorDialog("Fail to preprocess!")
            self._isBusy = False
            return
        ...
        try:
            if not action.Generate():
                self._isBusy = False
                self.LogMessage("Fail to generate package document!")
        except:
            self._isBusy = False
            self._ErrorDialog("Fail to generate package document!")
    ...
    def CreateDoxygeProcess(self, doxPath, configFile):
        ...
        try:
            ...
            self._ErrorDialog('Fail to launch doxygen cmd %s!' % cmd)
            return False          
    ...
    def DocumentFixup(self):
        ...
        for root, dirs, files in os.walk(self.GetParent().GetOutputPath()):
            for dir in dirs:
