...
class DoxygenAction:
    ...
    def Generate(self):
        ...
        if self._doxygenCallback != None:
            ...
            self.Log("Fail to create doxygen process!")
            return False
...
class PackageDocumentAction(DoxygenAction):
    ...
    def GenerateIndexPage(self):
        ...
        pdObj.AddPages(self.GenerateIncludesSubPage(self._pObj, self._configFile))
        pdObj.AddPages(self.GenerateLibraryClassesSubPage(self._pObj, self._configFile))
        pdObj.AddPages(self.GeneratePcdSubPages(self._pObj, self._configFile))
        pdObj.AddPages(self.GenerateGuidSubPages(self._pObj, self._configFile))
        pdObj.AddPages(self.GeneratePpiSubPages(self._pObj, self._configFile))
        pdObj.AddPages(self.GenerateProtocolSubPages(self._pObj, self._configFile))
        ...
        for obj in objs:
            ...
            for file in os.listdir(path):
                ...
                if os.path.isfile(fullpath):
                    ...
                    if file.lower() in ['library', 'protocol', 'guid', 'ppi', 'ia32', 'x64', 'ipf', 'ebc', 'pi', 'uefi']:
                        continue
    ...
    def ProcessSourceFileForInclude(self, path, pObj, configFile, infObj=None):
        ...
        for no in xrange(len(lines)):
            ...
            #dir = os.path.dirname(mObj.GetFilename())
            fullPath = None
            ...
            if os.path.exists(os.path.join(os.path.dirname(path), filePath)):
                ...
                for incObj in incObjs:
                    ...
                    incPath = os.path.join(incPath, filePath)
            ...
            if fullPath == None and self.IsVerbose():
                self.Log('Can not resolve header file %s for file %s\n' % (filePath, path))
                #ErrorMsg ('Can not resolve header file %s for file %s' % (filePath, path))
                return
            else:
    ...
    def GenerateModulePages(self, pObj, configFile):
        ...
        if len(libObjs) != 0:
            libRootPage = doxygen.Page('Libaries', 'lib_root_page')
    ...
    def GenerateModulePage(self, pObj, infObj, configFile, isLib):
        ...
        for lcObj in infObj.GetSectionObjectsByName('libraryclasses', self._arch):
            ...
            pkgname, hPath = self.SearchLibraryClassHeaderFile(lcObj.GetClass(),
                                                              workspace,
                                                              refDecObjs)
    ...
    def TranslateUniFile(self, path):
        ...
        if os.path.exists(newpath):
            os.remove(newpath)
        file = textfile.TextFile(newpath)
        file.Write(output)
        file.Close()
        return newpath
